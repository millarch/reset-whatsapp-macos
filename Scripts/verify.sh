...
[[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$APP/Contents/Info.plist")" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]
/usr/bin/file "$APP/Contents/MacOS/reset-whatsapp" | /usr/bin/grep -Eq 'Mach-O 64-bit executable (arm64|x86_64)'
